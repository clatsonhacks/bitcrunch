...
export async function POST(req) {
  ...
  const category = `1. Asked question is related to NFT collection, 2. Asked question related to NFT price estimation, 3. Asked question related to NFT gaming metrics, 4.The question is related to Top deals in NFT, 5. Other info related to NFT`;
  ...
  try {
    ...
    if (categoryNumber === "1") {
      ...
      ansprompt = `According to the prompt "${prompt}", provide answer using the following data "${JSON.stringify(categoryData)}.Ensure any links in the response are clickable using markdown format like [Link Text](URL).Provide the answers in a descriptive format.Also don't provide the entire json info.Just select related to question"`;
    }
    else if (categoryNumber === "5") {
      categoryData = await handleCategory("5");
